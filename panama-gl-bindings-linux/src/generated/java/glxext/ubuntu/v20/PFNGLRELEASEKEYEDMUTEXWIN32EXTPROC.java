...
public interface PFNGLRELEASEKEYEDMUTEXWIN32EXTPROC {
    ...
    static MemorySegment allocate(PFNGLRELEASEKEYEDMUTEXWIN32EXTPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLRELEASEKEYEDMUTEXWIN32EXTPROC.class, fi, constants$782.PFNGLRELEASEKEYEDMUTEXWIN32EXTPROC$FUNC, session);
    }
    static PFNGLRELEASEKEYEDMUTEXWIN32EXTPROC ofAddress(MemoryAddress addr, MemorySession session) {
        ...
        return (int _memory, long _key) -> {
            try {
                return (byte)constants$782.PFNGLRELEASEKEYEDMUTEXWIN32EXTPROC$MH.invokeExact((Addressable)symbol, _memory, _key);
            } catch (Throwable ex$) {
