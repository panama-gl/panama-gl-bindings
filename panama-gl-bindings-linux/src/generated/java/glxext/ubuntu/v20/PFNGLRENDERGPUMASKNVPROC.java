...
public interface PFNGLRENDERGPUMASKNVPROC {
    ...
    static MemorySegment allocate(PFNGLRENDERGPUMASKNVPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLRENDERGPUMASKNVPROC.class, fi, constants$839.PFNGLRENDERGPUMASKNVPROC$FUNC, session);
    }
    static PFNGLRENDERGPUMASKNVPROC ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return (int _mask) -> {
            try {
                constants$839.PFNGLRENDERGPUMASKNVPROC$MH.invokeExact((Addressable)symbol, _mask);
            } catch (Throwable ex$) {
