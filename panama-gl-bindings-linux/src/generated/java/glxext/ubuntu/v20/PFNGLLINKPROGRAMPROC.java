...
public interface PFNGLLINKPROGRAMPROC {
    ...
    static MemorySegment allocate(PFNGLLINKPROGRAMPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLLINKPROGRAMPROC.class, fi, constants$209.PFNGLLINKPROGRAMPROC$FUNC, session);
    }
    static PFNGLLINKPROGRAMPROC ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return (int _program) -> {
            try {
                constants$209.PFNGLLINKPROGRAMPROC$MH.invokeExact((Addressable)symbol, _program);
            } catch (Throwable ex$) {
