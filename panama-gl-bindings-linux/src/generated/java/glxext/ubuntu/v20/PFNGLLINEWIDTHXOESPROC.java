...
public interface PFNGLLINEWIDTHXOESPROC {
    ...
    static MemorySegment allocate(PFNGLLINEWIDTHXOESPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLLINEWIDTHXOESPROC.class, fi, constants$504.PFNGLLINEWIDTHXOESPROC$FUNC, session);
    }
    static PFNGLLINEWIDTHXOESPROC ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return (int _width) -> {
            try {
                constants$504.PFNGLLINEWIDTHXOESPROC$MH.invokeExact((Addressable)symbol, _width);
            } catch (Throwable ex$) {
