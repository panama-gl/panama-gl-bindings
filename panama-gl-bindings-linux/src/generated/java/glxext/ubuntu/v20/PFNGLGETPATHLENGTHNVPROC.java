...
public interface PFNGLGETPATHLENGTHNVPROC {
    ...
    static MemorySegment allocate(PFNGLGETPATHLENGTHNVPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLGETPATHLENGTHNVPROC.class, fi, constants$885.PFNGLGETPATHLENGTHNVPROC$FUNC, session);
    }
    static PFNGLGETPATHLENGTHNVPROC ofAddress(MemoryAddress addr, MemorySession session) {
        ...
        return (int _path, int _startSegment, int _numSegments) -> {
            try {
                return (float)constants$885.PFNGLGETPATHLENGTHNVPROC$MH.invokeExact((Addressable)symbol, _path, _startSegment, _numSegments);
            } catch (Throwable ex$) {
