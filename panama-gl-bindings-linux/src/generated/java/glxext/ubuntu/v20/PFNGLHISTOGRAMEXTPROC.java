...
public interface PFNGLHISTOGRAMEXTPROC {
    ...
    static MemorySegment allocate(PFNGLHISTOGRAMEXTPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLHISTOGRAMEXTPROC.class, fi, constants$723.PFNGLHISTOGRAMEXTPROC$FUNC, session);
    }
    static PFNGLHISTOGRAMEXTPROC ofAddress(MemoryAddress addr, MemorySession session) {
        ...
        return (int _target, int _width, int _internalformat, byte _sink) -> {
            try {
                constants$723.PFNGLHISTOGRAMEXTPROC$MH.invokeExact((Addressable)symbol, _target, _width, _internalformat, _sink);
            } catch (Throwable ex$) {
