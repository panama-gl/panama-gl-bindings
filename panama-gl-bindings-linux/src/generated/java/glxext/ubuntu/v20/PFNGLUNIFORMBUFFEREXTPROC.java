// Generated by jextract

package glxext.ubuntu.v20;

import java.lang.invoke.MethodHandle;
import java.lang.invoke.VarHandle;
import java.nio.ByteOrder;
import java.lang.foreign.*;
import static java.lang.foreign.ValueLayout.*;
public interface PFNGLUNIFORMBUFFEREXTPROC {

    void apply(int program, int location, int buffer);
    static MemorySegment allocate(PFNGLUNIFORMBUFFEREXTPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLUNIFORMBUFFEREXTPROC.class, fi, constants$601.PFNGLUNIFORMBUFFEREXTPROC$FUNC, session);
    }
    static PFNGLUNIFORMBUFFEREXTPROC ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return (int _program, int _location, int _buffer) -> {
            try {
                constants$601.PFNGLUNIFORMBUFFEREXTPROC$MH.invokeExact((Addressable)symbol, _program, _location, _buffer);
            } catch (Throwable ex$) {
                throw new AssertionError("should not reach here", ex$);
            }
        };
    }
}


