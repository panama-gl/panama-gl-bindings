...
public interface PFNGLCREATESHADEROBJECTARBPROC {
    ...
    static MemorySegment allocate(PFNGLCREATESHADEROBJECTARBPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLCREATESHADEROBJECTARBPROC.class, fi, constants$444.PFNGLCREATESHADEROBJECTARBPROC$FUNC, session);
    }
    static PFNGLCREATESHADEROBJECTARBPROC ofAddress(MemoryAddress addr, MemorySession session) {
        ...
        return (int _shaderType) -> {
            try {
                return (int)constants$444.PFNGLCREATESHADEROBJECTARBPROC$MH.invokeExact((Addressable)symbol, _shaderType);
            } catch (Throwable ex$) {
