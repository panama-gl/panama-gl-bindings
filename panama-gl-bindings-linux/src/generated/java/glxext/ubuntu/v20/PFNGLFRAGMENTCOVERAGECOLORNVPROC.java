...
public interface PFNGLFRAGMENTCOVERAGECOLORNVPROC {
    ...
    static MemorySegment allocate(PFNGLFRAGMENTCOVERAGECOLORNVPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLFRAGMENTCOVERAGECOLORNVPROC.class, fi, constants$834.PFNGLFRAGMENTCOVERAGECOLORNVPROC$FUNC, session);
    }
    static PFNGLFRAGMENTCOVERAGECOLORNVPROC ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return (int _color) -> {
            try {
                constants$834.PFNGLFRAGMENTCOVERAGECOLORNVPROC$MH.invokeExact((Addressable)symbol, _color);
            } catch (Throwable ex$) {
