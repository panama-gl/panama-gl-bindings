...
public interface PFNGLVERTEXBLENDARBPROC {
    ...
    static MemorySegment allocate(PFNGLVERTEXBLENDARBPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLVERTEXBLENDARBPROC.class, fi, constants$466.PFNGLVERTEXBLENDARBPROC$FUNC, session);
    }
    static PFNGLVERTEXBLENDARBPROC ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return (int _count) -> {
            try {
                constants$466.PFNGLVERTEXBLENDARBPROC$MH.invokeExact((Addressable)symbol, _count);
            } catch (Throwable ex$) {
