...
public interface PFNGLCOMPILECOMMANDLISTNVPROC {
    ...
    static MemorySegment allocate(PFNGLCOMPILECOMMANDLISTNVPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLCOMPILECOMMANDLISTNVPROC.class, fi, constants$822.PFNGLCOMPILECOMMANDLISTNVPROC$FUNC, session);
    }
    static PFNGLCOMPILECOMMANDLISTNVPROC ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return (int _list) -> {
            try {
                constants$822.PFNGLCOMPILECOMMANDLISTNVPROC$MH.invokeExact((Addressable)symbol, _list);
            } catch (Throwable ex$) {
