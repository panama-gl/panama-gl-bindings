...
public interface PFNGLMAXSHADERCOMPILERTHREADSARBPROC {
    ...
    static MemorySegment allocate(PFNGLMAXSHADERCOMPILERTHREADSARBPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLMAXSHADERCOMPILERTHREADSARBPROC.class, fi, constants$434.PFNGLMAXSHADERCOMPILERTHREADSARBPROC$FUNC, session);
    }
    static PFNGLMAXSHADERCOMPILERTHREADSARBPROC ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return (int _count) -> {
            try {
                constants$434.PFNGLMAXSHADERCOMPILERTHREADSARBPROC$MH.invokeExact((Addressable)symbol, _count);
            } catch (Throwable ex$) {
