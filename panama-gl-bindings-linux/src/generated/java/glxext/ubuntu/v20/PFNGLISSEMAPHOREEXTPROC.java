...
public interface PFNGLISSEMAPHOREEXTPROC {
    ...
    static MemorySegment allocate(PFNGLISSEMAPHOREEXTPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLISSEMAPHOREEXTPROC.class, fi, constants$746.PFNGLISSEMAPHOREEXTPROC$FUNC, session);
    }
    static PFNGLISSEMAPHOREEXTPROC ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return (int _semaphore) -> {
            try {
                return (byte)constants$746.PFNGLISSEMAPHOREEXTPROC$MH.invokeExact((Addressable)symbol, _semaphore);
            } catch (Throwable ex$) {
