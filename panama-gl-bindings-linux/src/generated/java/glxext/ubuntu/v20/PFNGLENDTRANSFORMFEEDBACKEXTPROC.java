...
public interface PFNGLENDTRANSFORMFEEDBACKEXTPROC {
    ...
    static MemorySegment allocate(PFNGLENDTRANSFORMFEEDBACKEXTPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLENDTRANSFORMFEEDBACKEXTPROC.class, fi, constants$759.PFNGLENDTRANSFORMFEEDBACKEXTPROC$FUNC, session);
    }
    static PFNGLENDTRANSFORMFEEDBACKEXTPROC ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return () -> {
            try {
                constants$759.PFNGLENDTRANSFORMFEEDBACKEXTPROC$MH.invokeExact((Addressable)symbol);
            } catch (Throwable ex$) {
