...
public interface PFNGLBLENDFUNCIARBPROC {
    ...
    static MemorySegment allocate(PFNGLBLENDFUNCIARBPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLBLENDFUNCIARBPROC.class, fi, constants$407.PFNGLBLENDFUNCIARBPROC$FUNC, session);
    }
    static PFNGLBLENDFUNCIARBPROC ofAddress(MemoryAddress addr, MemorySession session) {
        ...
        return (int _buf, int _src, int _dst) -> {
            try {
                constants$407.PFNGLBLENDFUNCIARBPROC$MH.invokeExact((Addressable)symbol, _buf, _src, _dst);
            } catch (Throwable ex$) {
