...
public interface PFNGLMULTITEXGENFEXTPROC {
    ...
    static MemorySegment allocate(PFNGLMULTITEXGENFEXTPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLMULTITEXGENFEXTPROC.class, fi, constants$636.PFNGLMULTITEXGENFEXTPROC$FUNC, session);
    }
    static PFNGLMULTITEXGENFEXTPROC ofAddress(MemoryAddress addr, MemorySession session) {
        ...
        return (int _texunit, int _coord, int _pname, float _param) -> {
            try {
                constants$636.PFNGLMULTITEXGENFEXTPROC$MH.invokeExact((Addressable)symbol, _texunit, _coord, _pname, _param);
            } catch (Throwable ex$) {
