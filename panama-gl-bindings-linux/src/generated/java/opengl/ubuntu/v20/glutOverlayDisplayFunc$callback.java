...
public interface glutOverlayDisplayFunc$callback {
    ...
    static MemorySegment allocate(glutOverlayDisplayFunc$callback fi, MemorySession session) {
        return RuntimeHelper.upcallStub(glutOverlayDisplayFunc$callback.class, fi, constants$957.glutOverlayDisplayFunc$callback$FUNC, session);
    }
    static glutOverlayDisplayFunc$callback ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return () -> {
            try {
                constants$957.glutOverlayDisplayFunc$callback$MH.invokeExact((Addressable)symbol);
            } catch (Throwable ex$) {
