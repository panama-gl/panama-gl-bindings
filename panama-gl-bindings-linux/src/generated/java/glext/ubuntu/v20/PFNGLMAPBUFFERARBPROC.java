...
public interface PFNGLMAPBUFFERARBPROC {
    ...
    static MemorySegment allocate(PFNGLMAPBUFFERARBPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLMAPBUFFERARBPROC.class, fi, constants$528.PFNGLMAPBUFFERARBPROC$FUNC, session);
    }
    static PFNGLMAPBUFFERARBPROC ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return (int _target, int _access) -> {
            try {
                return (java.lang.foreign.Addressable)(java.lang.foreign.MemoryAddress)constants$528.PFNGLMAPBUFFERARBPROC$MH.invokeExact((Addressable)symbol, _target, _access);
            } catch (Throwable ex$) {
