...
public interface PFNGLFENCESYNCPROC {
    ...
    static MemorySegment allocate(PFNGLFENCESYNCPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLFENCESYNCPROC.class, fi, constants$223.PFNGLFENCESYNCPROC$FUNC, session);
    }
    static PFNGLFENCESYNCPROC ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return (int _condition, int _flags) -> {
            try {
                return (java.lang.foreign.Addressable)(java.lang.foreign.MemoryAddress)constants$223.PFNGLFENCESYNCPROC$MH.invokeExact((Addressable)symbol, _condition, _flags);
            } catch (Throwable ex$) {
