...
public interface PFNGLPROVOKINGVERTEXEXTPROC {
    ...
    static MemorySegment allocate(PFNGLPROVOKINGVERTEXEXTPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLPROVOKINGVERTEXEXTPROC.class, fi, constants$934.PFNGLPROVOKINGVERTEXEXTPROC$FUNC, session);
    }
    static PFNGLPROVOKINGVERTEXEXTPROC ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return (int _mode) -> {
            try {
                constants$934.PFNGLPROVOKINGVERTEXEXTPROC$MH.invokeExact((Addressable)symbol, _mode);
            } catch (Throwable ex$) {
