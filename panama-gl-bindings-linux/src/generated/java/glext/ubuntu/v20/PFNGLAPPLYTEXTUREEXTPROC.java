...
public interface PFNGLAPPLYTEXTUREEXTPROC {
    ...
    static MemorySegment allocate(PFNGLAPPLYTEXTUREEXTPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLAPPLYTEXTUREEXTPROC.class, fi, constants$913.PFNGLAPPLYTEXTUREEXTPROC$FUNC, session);
    }
    static PFNGLAPPLYTEXTUREEXTPROC ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return (int _mode) -> {
            try {
                constants$913.PFNGLAPPLYTEXTUREEXTPROC$MH.invokeExact((Addressable)symbol, _mode);
            } catch (Throwable ex$) {
