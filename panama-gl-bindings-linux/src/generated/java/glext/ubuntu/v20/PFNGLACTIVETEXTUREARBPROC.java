...
public interface PFNGLACTIVETEXTUREARBPROC {
    ...
    static MemorySegment allocate(PFNGLACTIVETEXTUREARBPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLACTIVETEXTUREARBPROC.class, fi, constants$80.PFNGLACTIVETEXTUREARBPROC$FUNC, session);
    }
    static PFNGLACTIVETEXTUREARBPROC ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return (int _texture) -> {
            try {
                constants$80.PFNGLACTIVETEXTUREARBPROC$MH.invokeExact((Addressable)symbol, _texture);
            } catch (Throwable ex$) {
