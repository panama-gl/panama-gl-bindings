...
public interface PFNGLPOINTSIZEXOESPROC {
    ...
    static MemorySegment allocate(PFNGLPOINTSIZEXOESPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLPOINTSIZEXOESPROC.class, fi, constants$582.PFNGLPOINTSIZEXOESPROC$FUNC, session);
    }
    static PFNGLPOINTSIZEXOESPROC ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return (int _size) -> {
            try {
                constants$583.PFNGLPOINTSIZEXOESPROC$MH.invokeExact((Addressable)symbol, _size);
            } catch (Throwable ex$) {
