...
public interface PFNGLBEGINOCCLUSIONQUERYNVPROC {
    ...
    static MemorySegment allocate(PFNGLBEGINOCCLUSIONQUERYNVPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLBEGINOCCLUSIONQUERYNVPROC.class, fi, constants$1128.PFNGLBEGINOCCLUSIONQUERYNVPROC$FUNC, session);
    }
    static PFNGLBEGINOCCLUSIONQUERYNVPROC ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return (int _id) -> {
            try {
                constants$1129.PFNGLBEGINOCCLUSIONQUERYNVPROC$MH.invokeExact((Addressable)symbol, _id);
            } catch (Throwable ex$) {
