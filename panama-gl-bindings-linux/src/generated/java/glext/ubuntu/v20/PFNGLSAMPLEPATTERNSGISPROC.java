...
public interface PFNGLSAMPLEPATTERNSGISPROC {
    ...
    static MemorySegment allocate(PFNGLSAMPLEPATTERNSGISPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLSAMPLEPATTERNSGISPROC.class, fi, constants$1281.PFNGLSAMPLEPATTERNSGISPROC$FUNC, session);
    }
    static PFNGLSAMPLEPATTERNSGISPROC ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return (int _pattern) -> {
            try {
                constants$1281.PFNGLSAMPLEPATTERNSGISPROC$MH.invokeExact((Addressable)symbol, _pattern);
            } catch (Throwable ex$) {
