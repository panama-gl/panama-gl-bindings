// Generated by jextract

package glext.ubuntu.v20;

import java.lang.invoke.MethodHandle;
import java.lang.invoke.VarHandle;
import java.nio.ByteOrder;
import java.lang.foreign.*;
import static java.lang.foreign.ValueLayout.*;
public interface PFNGLISVERTEXARRAYPROC {

    byte apply(int array);
    static MemorySegment allocate(PFNGLISVERTEXARRAYPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLISVERTEXARRAYPROC.class, fi, constants$201.PFNGLISVERTEXARRAYPROC$FUNC, session);
    }
    static PFNGLISVERTEXARRAYPROC ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return (int _array) -> {
            try {
                return (byte)constants$201.PFNGLISVERTEXARRAYPROC$MH.invokeExact((Addressable)symbol, _array);
            } catch (Throwable ex$) {
                throw new AssertionError("should not reach here", ex$);
            }
        };
    }
}


