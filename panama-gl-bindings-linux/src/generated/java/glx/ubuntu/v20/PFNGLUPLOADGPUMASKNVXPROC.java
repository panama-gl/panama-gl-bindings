...
public interface PFNGLUPLOADGPUMASKNVXPROC {
    ...
    static MemorySegment allocate(PFNGLUPLOADGPUMASKNVXPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLUPLOADGPUMASKNVXPROC.class, fi, constants$806.PFNGLUPLOADGPUMASKNVXPROC$FUNC, session);
    }
    static PFNGLUPLOADGPUMASKNVXPROC ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return (int _mask) -> {
            try {
                constants$806.PFNGLUPLOADGPUMASKNVXPROC$MH.invokeExact((Addressable)symbol, _mask);
            } catch (Throwable ex$) {
