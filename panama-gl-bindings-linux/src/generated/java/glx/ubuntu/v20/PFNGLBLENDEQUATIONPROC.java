...
public interface PFNGLBLENDEQUATIONPROC {
    ...
    static MemorySegment allocate(PFNGLBLENDEQUATIONPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLBLENDEQUATIONPROC.class, fi, constants$192.PFNGLBLENDEQUATIONPROC$FUNC, session);
    }
    static PFNGLBLENDEQUATIONPROC ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return (int _mode) -> {
            try {
                constants$192.PFNGLBLENDEQUATIONPROC$MH.invokeExact((Addressable)symbol, _mode);
            } catch (Throwable ex$) {
