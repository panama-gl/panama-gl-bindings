...
public interface PFNGLPRIMITIVERESTARTINDEXPROC {
    ...
    static MemorySegment allocate(PFNGLPRIMITIVERESTARTINDEXPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLPRIMITIVERESTARTINDEXPROC.class, fi, constants$260.PFNGLPRIMITIVERESTARTINDEXPROC$FUNC, session);
    }
    static PFNGLPRIMITIVERESTARTINDEXPROC ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return (int _index) -> {
            try {
                constants$260.PFNGLPRIMITIVERESTARTINDEXPROC$MH.invokeExact((Addressable)symbol, _index);
            } catch (Throwable ex$) {
