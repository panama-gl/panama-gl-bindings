...
public interface PFNGLREADINSTRUMENTSSGIXPROC {
    ...
    static MemorySegment allocate(PFNGLREADINSTRUMENTSSGIXPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLREADINSTRUMENTSSGIXPROC.class, fi, constants$986.PFNGLREADINSTRUMENTSSGIXPROC$FUNC, session);
    }
    static PFNGLREADINSTRUMENTSSGIXPROC ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return (int _marker) -> {
            try {
                constants$986.PFNGLREADINSTRUMENTSSGIXPROC$MH.invokeExact((Addressable)symbol, _marker);
            } catch (Throwable ex$) {
