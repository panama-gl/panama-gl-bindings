...
public interface PFNGLOBJECTUNPURGEABLEAPPLEPROC {
    ...
    static MemorySegment allocate(PFNGLOBJECTUNPURGEABLEAPPLEPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLOBJECTUNPURGEABLEAPPLEPROC.class, fi, constants$565.PFNGLOBJECTUNPURGEABLEAPPLEPROC$FUNC, session);
    }
    static PFNGLOBJECTUNPURGEABLEAPPLEPROC ofAddress(MemoryAddress addr, MemorySession session) {
        ...
        return (int _objectType, int _name, int _option) -> {
            try {
                return (int)constants$565.PFNGLOBJECTUNPURGEABLEAPPLEPROC$MH.invokeExact((Addressable)symbol, _objectType, _name, _option);
            } catch (Throwable ex$) {
