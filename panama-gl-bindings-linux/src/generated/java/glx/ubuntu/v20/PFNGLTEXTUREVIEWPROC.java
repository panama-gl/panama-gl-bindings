...
public interface PFNGLTEXTUREVIEWPROC {
    ...
    static MemorySegment allocate(PFNGLTEXTUREVIEWPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLTEXTUREVIEWPROC.class, fi, constants$347.PFNGLTEXTUREVIEWPROC$FUNC, session);
    }
    static PFNGLTEXTUREVIEWPROC ofAddress(MemoryAddress addr, MemorySession session) {
        ...
        return (int _texture, int _target, int _origtexture, int _internalformat, int _minlevel, int _numlevels, int _minlayer, int _numlayers) -> {
            try {
                constants$347.PFNGLTEXTUREVIEWPROC$MH.invokeExact((Addressable)symbol, _texture, _target, _origtexture, _internalformat, _minlevel, _numlevels, _minlayer, _numlayers);
            } catch (Throwable ex$) {
