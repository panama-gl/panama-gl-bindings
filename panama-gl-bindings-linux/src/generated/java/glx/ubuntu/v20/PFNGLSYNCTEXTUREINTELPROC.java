...
public interface PFNGLSYNCTEXTUREINTELPROC {
    ...
    static MemorySegment allocate(PFNGLSYNCTEXTUREINTELPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLSYNCTEXTUREINTELPROC.class, fi, constants$790.PFNGLSYNCTEXTUREINTELPROC$FUNC, session);
    }
    static PFNGLSYNCTEXTUREINTELPROC ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return (int _texture) -> {
            try {
                constants$790.PFNGLSYNCTEXTUREINTELPROC$MH.invokeExact((Addressable)symbol, _texture);
            } catch (Throwable ex$) {
