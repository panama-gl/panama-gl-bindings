...
public interface PFNGLISNAMEAMDPROC {
    ...
    static MemorySegment allocate(PFNGLISNAMEAMDPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLISNAMEAMDPROC.class, fi, constants$554.PFNGLISNAMEAMDPROC$FUNC, session);
    }
    static PFNGLISNAMEAMDPROC ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return (int _identifier, int _name) -> {
            try {
                return (byte)constants$554.PFNGLISNAMEAMDPROC$MH.invokeExact((Addressable)symbol, _identifier, _name);
            } catch (Throwable ex$) {
