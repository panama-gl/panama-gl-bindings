...
public interface PFNGLACTIVETEXTUREPROC {
    ...
    static MemorySegment allocate(PFNGLACTIVETEXTUREPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLACTIVETEXTUREPROC.class, fi, constants$156.PFNGLACTIVETEXTUREPROC$FUNC, session);
    }
    static PFNGLACTIVETEXTUREPROC ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return (int _texture) -> {
            try {
                constants$156.PFNGLACTIVETEXTUREPROC$MH.invokeExact((Addressable)symbol, _texture);
            } catch (Throwable ex$) {
