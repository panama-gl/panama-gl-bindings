...
public interface PFNGLISENABLEDIPROC {
    ...
    static MemorySegment allocate(PFNGLISENABLEDIPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLISENABLEDIPROC.class, fi, constants$233.PFNGLISENABLEDIPROC$FUNC, session);
    }
    static PFNGLISENABLEDIPROC ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return (int _target, int _index) -> {
            try {
                return (byte)constants$233.PFNGLISENABLEDIPROC$MH.invokeExact((Addressable)symbol, _target, _index);
            } catch (Throwable ex$) {
