...
public interface PFNGLENABLEVERTEXATTRIBARRAYPROC {
    ...
    static MemorySegment allocate(PFNGLENABLEVERTEXATTRIBARRAYPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLENABLEVERTEXATTRIBARRAYPROC.class, fi, constants$203.PFNGLENABLEVERTEXATTRIBARRAYPROC$FUNC, session);
    }
    static PFNGLENABLEVERTEXATTRIBARRAYPROC ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return (int _index) -> {
            try {
                constants$203.PFNGLENABLEVERTEXATTRIBARRAYPROC$MH.invokeExact((Addressable)symbol, _index);
            } catch (Throwable ex$) {
