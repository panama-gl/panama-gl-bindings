...
public interface PFNGLDISABLEVERTEXATTRIBARRAYARBPROC {
    ...
    static MemorySegment allocate(PFNGLDISABLEVERTEXATTRIBARRAYARBPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLDISABLEVERTEXATTRIBARRAYARBPROC.class, fi, constants$483.PFNGLDISABLEVERTEXATTRIBARRAYARBPROC$FUNC, session);
    }
    static PFNGLDISABLEVERTEXATTRIBARRAYARBPROC ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return (int _index) -> {
            try {
                constants$483.PFNGLDISABLEVERTEXATTRIBARRAYARBPROC$MH.invokeExact((Addressable)symbol, _index);
            } catch (Throwable ex$) {
