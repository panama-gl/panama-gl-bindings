...
public interface PFNGLMATRIXPUSHEXTPROC {
    ...
    static MemorySegment allocate(PFNGLMATRIXPUSHEXTPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLMATRIXPUSHEXTPROC.class, fi, constants$626.PFNGLMATRIXPUSHEXTPROC$FUNC, session);
    }
    static PFNGLMATRIXPUSHEXTPROC ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return (int _mode) -> {
            try {
                constants$626.PFNGLMATRIXPUSHEXTPROC$MH.invokeExact((Addressable)symbol, _mode);
            } catch (Throwable ex$) {
