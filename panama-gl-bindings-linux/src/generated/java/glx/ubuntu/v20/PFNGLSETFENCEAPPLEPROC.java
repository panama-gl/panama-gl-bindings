...
public interface PFNGLSETFENCEAPPLEPROC {
    ...
    static MemorySegment allocate(PFNGLSETFENCEAPPLEPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLSETFENCEAPPLEPROC.class, fi, constants$562.PFNGLSETFENCEAPPLEPROC$FUNC, session);
    }
    static PFNGLSETFENCEAPPLEPROC ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return (int _fence) -> {
            try {
                constants$562.PFNGLSETFENCEAPPLEPROC$MH.invokeExact((Addressable)symbol, _fence);
            } catch (Throwable ex$) {
