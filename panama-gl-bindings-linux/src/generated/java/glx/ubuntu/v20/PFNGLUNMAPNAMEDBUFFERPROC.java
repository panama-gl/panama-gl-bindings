...
public interface PFNGLUNMAPNAMEDBUFFERPROC {
    ...
    static MemorySegment allocate(PFNGLUNMAPNAMEDBUFFERPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLUNMAPNAMEDBUFFERPROC.class, fi, constants$361.PFNGLUNMAPNAMEDBUFFERPROC$FUNC, session);
    }
    static PFNGLUNMAPNAMEDBUFFERPROC ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return (int _buffer) -> {
            try {
                return (byte)constants$361.PFNGLUNMAPNAMEDBUFFERPROC$MH.invokeExact((Addressable)symbol, _buffer);
            } catch (Throwable ex$) {
