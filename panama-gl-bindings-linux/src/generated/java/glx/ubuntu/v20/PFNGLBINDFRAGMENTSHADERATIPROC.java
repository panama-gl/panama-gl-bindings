...
public interface PFNGLBINDFRAGMENTSHADERATIPROC {
    ...
    static MemorySegment allocate(PFNGLBINDFRAGMENTSHADERATIPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLBINDFRAGMENTSHADERATIPROC.class, fi, constants$574.PFNGLBINDFRAGMENTSHADERATIPROC$FUNC, session);
    }
    static PFNGLBINDFRAGMENTSHADERATIPROC ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return (int _id) -> {
            try {
                constants$574.PFNGLBINDFRAGMENTSHADERATIPROC$MH.invokeExact((Addressable)symbol, _id);
            } catch (Throwable ex$) {
