...
public interface PFNGLMAKETEXTUREHANDLENONRESIDENTNVPROC {
    ...
    static MemorySegment allocate(PFNGLMAKETEXTUREHANDLENONRESIDENTNVPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLMAKETEXTUREHANDLENONRESIDENTNVPROC.class, fi, constants$813.PFNGLMAKETEXTUREHANDLENONRESIDENTNVPROC$FUNC, session);
    }
    static PFNGLMAKETEXTUREHANDLENONRESIDENTNVPROC ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return (long _handle) -> {
            try {
                constants$813.PFNGLMAKETEXTUREHANDLENONRESIDENTNVPROC$MH.invokeExact((Addressable)symbol, _handle);
            } catch (Throwable ex$) {
