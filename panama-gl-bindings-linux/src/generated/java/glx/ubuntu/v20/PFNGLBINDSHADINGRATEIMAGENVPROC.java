...
public interface PFNGLBINDSHADINGRATEIMAGENVPROC {
    ...
    static MemorySegment allocate(PFNGLBINDSHADINGRATEIMAGENVPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLBINDSHADINGRATEIMAGENVPROC.class, fi, constants$909.PFNGLBINDSHADINGRATEIMAGENVPROC$FUNC, session);
    }
    static PFNGLBINDSHADINGRATEIMAGENVPROC ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return (int _texture) -> {
            try {
                constants$909.PFNGLBINDSHADINGRATEIMAGENVPROC$MH.invokeExact((Addressable)symbol, _texture);
            } catch (Throwable ex$) {
