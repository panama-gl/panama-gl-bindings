...
public interface PFNGLISPROGRAMPIPELINEPROC {
    ...
    static MemorySegment allocate(PFNGLISPROGRAMPIPELINEPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLISPROGRAMPIPELINEPROC.class, fi, constants$309.PFNGLISPROGRAMPIPELINEPROC$FUNC, session);
    }
    static PFNGLISPROGRAMPIPELINEPROC ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return (int _pipeline) -> {
            try {
                return (byte)constants$309.PFNGLISPROGRAMPIPELINEPROC$MH.invokeExact((Addressable)symbol, _pipeline);
            } catch (Throwable ex$) {
