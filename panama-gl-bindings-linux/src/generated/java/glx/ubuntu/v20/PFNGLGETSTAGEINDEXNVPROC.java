...
public interface PFNGLGETSTAGEINDEXNVPROC {
    ...
    static MemorySegment allocate(PFNGLGETSTAGEINDEXNVPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLGETSTAGEINDEXNVPROC.class, fi, constants$819.PFNGLGETSTAGEINDEXNVPROC$FUNC, session);
    }
    static PFNGLGETSTAGEINDEXNVPROC ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return (int _shadertype) -> {
            try {
                return (short)constants$819.PFNGLGETSTAGEINDEXNVPROC$MH.invokeExact((Addressable)symbol, _shadertype);
            } catch (Throwable ex$) {
