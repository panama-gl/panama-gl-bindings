...
public interface PFNGLISVARIANTENABLEDEXTPROC {
    ...
    static MemorySegment allocate(PFNGLISVARIANTENABLEDEXTPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLISVARIANTENABLEDEXTPROC.class, fi, constants$777.PFNGLISVARIANTENABLEDEXTPROC$FUNC, session);
    }
    static PFNGLISVARIANTENABLEDEXTPROC ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return (int _id, int _cap) -> {
            try {
                return (byte)constants$777.PFNGLISVARIANTENABLEDEXTPROC$MH.invokeExact((Addressable)symbol, _id, _cap);
            } catch (Throwable ex$) {
