...
public interface PFNGLREPLACEMENTCODEUBSUNPROC {
    ...
    static MemorySegment allocate(PFNGLREPLACEMENTCODEUBSUNPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLREPLACEMENTCODEUBSUNPROC.class, fi, constants$999.PFNGLREPLACEMENTCODEUBSUNPROC$FUNC, session);
    }
    static PFNGLREPLACEMENTCODEUBSUNPROC ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return (byte _code) -> {
            try {
                constants$999.PFNGLREPLACEMENTCODEUBSUNPROC$MH.invokeExact((Addressable)symbol, _code);
            } catch (Throwable ex$) {
