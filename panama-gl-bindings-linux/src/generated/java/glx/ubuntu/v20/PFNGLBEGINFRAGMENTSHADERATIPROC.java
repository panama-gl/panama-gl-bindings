...
public interface PFNGLBEGINFRAGMENTSHADERATIPROC {
    ...
    static MemorySegment allocate(PFNGLBEGINFRAGMENTSHADERATIPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLBEGINFRAGMENTSHADERATIPROC.class, fi, constants$575.PFNGLBEGINFRAGMENTSHADERATIPROC$FUNC, session);
    }
    static PFNGLBEGINFRAGMENTSHADERATIPROC ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return () -> {
            try {
                constants$575.PFNGLBEGINFRAGMENTSHADERATIPROC$MH.invokeExact((Addressable)symbol);
            } catch (Throwable ex$) {
