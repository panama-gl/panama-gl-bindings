// Generated by jextract

package glx.ubuntu.v20;

import java.lang.invoke.MethodHandle;
import java.lang.invoke.VarHandle;
import java.nio.ByteOrder;
import java.lang.foreign.*;
import static java.lang.foreign.ValueLayout.*;
public interface PFNGLENDQUERYPROC {

    void apply(int target);
    static MemorySegment allocate(PFNGLENDQUERYPROC fi, MemorySession session) {
        return RuntimeHelper.upcallStub(PFNGLENDQUERYPROC.class, fi, constants$193.PFNGLENDQUERYPROC$FUNC, session);
    }
    static PFNGLENDQUERYPROC ofAddress(MemoryAddress addr, MemorySession session) {
        MemorySegment symbol = MemorySegment.ofAddress(addr, 0, session);
        return (int _target) -> {
            try {
                constants$193.PFNGLENDQUERYPROC$MH.invokeExact((Addressable)symbol, _target);
            } catch (Throwable ex$) {
                throw new AssertionError("should not reach here", ex$);
            }
        };
    }
}


